...
// - Added Clojure, Lisp, Elisp
//
module.exports = {
  // Couldn't find a way to import the match-braces Prismjs plugin
  // into this configuration.
  //
  // Tried including the plugin code into `init` but didn't work.
  // console.log(Prism.plugins) => {} so no plugins are imported.
  codeAttributes: {
    "class": function ({ language }) {
      let extra = ''
      if (language === 'lisp') {
        extra = ` match-braces rainbow-braces`
      }
      return `language-${language}${extra}`
    }
  },
      ...
      keyword: [
        {
          pattern: RegExp(
            par +
            '(?:and|(?:cl-)?letf|loop|cond|cons|error|if|(?:lexical-)?let\\*?|message|not|null|or|provide|require|setq|setf|getf|remf|gethash|remhash|aref|elt|vector-push-extend|vector-pop|coerce|makunbound|first|unless|nth|push|use-package|when|while)' +
            space
          ),
          lookbehind: true
        },
